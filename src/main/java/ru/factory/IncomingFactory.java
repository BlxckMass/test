package ru.factory;

import ru.exception.DocumentExistsException;
import ru.model.document.Document;
import ru.model.document.Incoming;
import ru.model.data.Storage;

import java.util.Date;
import java.util.Random;
import java.util.UUID;
import java.util.concurrent.ThreadLocalRandom;

public class IncomingFactory implements DocumentFactoryInterface {

    private final Random random = new Random();

    public IncomingFactory() {
    }

    @Override
    public Document createDocument() {
        Incoming document = new Incoming();
        Date date = new Date(Math.abs(ThreadLocalRandom.current().nextLong(0L, 1629284699L * 1000)));

        document.setId(Math.abs(random.nextLong()));
        document.setAuthor(Document.defaultValues.get(authors).get(random.nextInt(Document.defaultValues.get(authors).size())));
        document.setName(UUID.randomUUID().toString());
        document.setText("text " + UUID.randomUUID());
        document.setRegistrationNumber(Math.abs(random.nextLong()));
        document.setDateOfRegistration(date);

        document.setSender(Document.defaultValues.get(senders).get(random.nextInt(Document.defaultValues.get(senders).size())));
        document.setAddressee(Document.defaultValues.get(addressees).get(random.nextInt(Document.defaultValues.get(addressees).size())));
        document.setOutgoingNumber(Math.abs(random.nextLong()));
        document.setOutgoingDateOfRegistration(date);
        Storage storage = Storage.getStorage();
        try {
            storage.save(document, document.getAuthor());
            return document;
        } catch (DocumentExistsException e) {
            e.printStackTrace();
            return null;
        }
    }

}
